...
extern "C" void inline Verus2hash(unsigned char *hash, unsigned char *curBuf, uint32_t nonce,
	u128 *data_key, uint32_t *fixrand, uint32_t *fixrandex, u128 *g_prand, 
	u128 *g_prandex)
{
	...
	intermediate = verusclhashv2_2(data_key, curBuf, fixrand, fixrandex, g_prand, g_prandex);
		//FillExtra
...
extern "C" int scanhash_verus(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	u128 data_key[(VERUS_KEY_SIZE + 1024)/sizeof(u128)] = {0};
	...
	do {
		...
		*hashes_done = nonce_buf + throughput;
		Verus2hash((unsigned char *)vhash, (unsigned char *)blockhash_half, nonce_buf, data_key, fixrand, fixrandex , data_key_prand, data_key_prandex);
