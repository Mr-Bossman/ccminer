...
{
	alignas(128) uint8_t _pbuf_copy[4*16];

	pbuf_copy(0) = _mm_xor_si128(
			__m128i_u8_index(buf, 0),
			__m128i_u8_index(buf, 2));
	pbuf_copy(1) = _mm_xor_si128(
			__m128i_u8_index(buf, 1),
			__m128i_u8_index(buf, 3));
	pbuf_copy(2) = __m128i_u8_index(buf, 2);
	pbuf_copy(3) = __m128i_u8_index(buf, 3);
	const __m128i *pbuf;
...
//		__m128i_index(g_prandex, i) = *prandex;
		...
		case 0:
		{
			...
			const __m128i temp2 = __m128i_index(pbuf, (selector & 1) ? -1 : 1);
		...
		case 4:
		{
			...
			const __m128i temp22 = __m128i_index(pbuf, (selector & 1) ? -1 : 1);
			const __m128i add12 = _mm_xor_si128(temp12, temp22);
		...
		case 0x10:
		{
			...
			const uint8_t *_rc = (uint8_t*)prand;
			__m128i tmp;

			__m128i temp1 = __m128i_index(pbuf, (selector & 1) ? -1 : 1);
		...
		case 0x14:
		{
			...
			do
			{
				if (selector & (((uint64_t)0x10000000) << rounds))
				{
					...
					const __m128i add1 = _mm_xor_si128(rc(0), temp2);
					_rc += 16;
					const __m128i clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
					acc = _mm_xor_si128(clprod1, acc);
				}
				...
				{
					onekey = _mm_load_si128((__m128i *)_rc);
					_rc += 16;
		...
		case 0x18:
		{
			...
			do
			{
				if (selector & (((uint64_t)0x10000000) << rounds))
				{
					//	onekey = _mm_load_si128(rc++);
					const __m128i temp2 = _mm_load_si128(rounds & 1 ? pbuf : buftmp);
					onekey = _mm_xor_si128(rc(0), temp2);
					_rc += 16;
		...
		case 0x1c:
		{
			...
			const __m128i temp4 = __m128i_index(pbuf, (selector & 1) ? -1 : 1);
...
uint64_t verusclhashv2_2(void * random, const unsigned char buf[64], uint64_t keyMask, uint32_t *fixrand, uint32_t *fixrandex,
	...
	__m128i  acc = __verusclmulwithoutreduction64alignedrepeatv2_2((uint8_t *)random, buf, 511, fixrand, fixrandex, g_prand, g_prandex);
